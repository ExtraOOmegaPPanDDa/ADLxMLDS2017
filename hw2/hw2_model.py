    ...
    def __init__(self, n_vocabs, for_testing = False):
        ...
        if not for_testing:
            ...
            for i in range(self.max_sent_len):
                ...
                onehot_labels = tf.sparse_to_dense(concated, tf.stack([self.batch_size, self.n_vocabs]), 1.0, 0.0)
            ...
            feat_flat = tf.reshape(self.feat, [-1, self.feat_dim])
            encoding_input = tf.nn.xw_plus_b(feat_flat, self.encode_w, self.encode_b)
            ...
            for i in range(0,self.max_sent_len):
                if i == 0:
